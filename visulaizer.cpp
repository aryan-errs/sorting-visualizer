...
const int rectspace = 1;

vector<int> arr(vector_size);
vector<int> Barr(vector_size);
...
void loadArr() { copy(Barr.begin(), Barr.end(), arr.begin()); }

void randomizeAndSaveArray() {
  unsigned int seed = (unsigned)time(NULL);
  srand(seed);
  for (int i = 0; i < vector_size; i++) {
    int random = rand() % (screenHeight);
    Barr[i] = random;
  }
}
...
void inplaceHeapSort(vector<int> &input, int n) {
  ...
  for (int heapLast = n - 1; heapLast >= 0; heapLast--) {
    ...
    while (leftChildIndex < heapLast) {
      ...
      if (rightChildIndex < heapLast &&
          input[rightChildIndex] > input[maxIndex]) {
      ...
      while (SDL_PollEvent(&e) != 0) {
        if (e.type == SDL_QUIT) {
          ...
          case (SDLK_6):
            ...
            cout << "\nHEAP SORT STARTED.\n";
            sorted = false;
            inplaceHeapSort(arr, vector_size);
            sorted = false;
            cout << "\nHEAP SORT COMPLETE.\n";
            break;
          }
...
bool controls() {
  cout << "WARNING: Giving repetitive commands may cause latency and the "
          "visualizer may behave unexpectedly. Please give a new command only "
          "after the current command's execution is done.\n\n"
...
void intro() {
  cout << "==============================Sorting "
          "Visualizer==============================\n\n"
       << "Visualization of different sorting algorithms in C++ with SDL2 "
          "Library. A sorting algorithm is an algorithm that puts the elements "
          "of a list in a certain order. While there are a large number of "
          "sorting algorithms, in practical implementations a few algorithms "
          "predominate.\n"
       << "In this implementation of sorting visualizer, we'll be looking at "
          "some of these sorting algorithms and visually comprehend their "
          "working.\n"
       << "The sorting algorithms covered here are Selection Sort, Insertion "
          "Sort, Bubble Sort, Merge Sort, Quick Sort and Heap Sort.\n"
       << "The list size is fixed to 130 elements. You can randomize the list "
          "and select any type of sorting algorithm to call on the list from "
          "the given options. Here, all sorting algorithms will sort the "
          "elements in ascending order. The sorting time being visualized for "
          "an algorithm is not exactly same as their actual time complexities. "
          "The relatively faster algorithms like Merge Sort, etc. have been "
          "delayed so that they could be properly visualized.\n\n"
